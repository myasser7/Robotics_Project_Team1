...
q1_traj_deg = zeros(1, length(t));
q2_traj_deg = zeros(1, length(t));
q3_traj_deg = zeros(1, length(t));

% Initial guess for joint angles (example: zero configuration)
q1_guess = 0; q2_guess = 2; q3_guess = 2;
...
for i = 1:10:length(t)
    ...
    q1_traj_deg(i) = q1_traj(i);
    q2_traj_deg(i) = q2_traj(i);
    q3_traj_deg(i) = q3_traj(i);

    q1_traj(i) = deg2rad(q1_traj(i));
    q2_traj(i) = deg2rad(q2_traj(i));
    q3_traj(i) = deg2rad(q3_traj(i));
    % Joint positions
    x1 = 0; y1 = 0; z1 = L1;          % Base to joint 1
    x2 = L2 * cos(q2_traj(i)) * cos(q1_traj(i));
    y2 = L2 * cos(q2_traj(i)) * sin(q1_traj(i));
    z2 = L1 + L2 * sin(q2_traj(i));
    x3 = x2 + L3 * cos(q2_traj(i) + q3_traj(i)) * cos(q1_traj(i));
    y3 = y2 + L3 * cos(q2_traj(i) + q3_traj(i)) * sin(q1_traj(i));
    z3 = z2 + L3 * sin(q2_traj(i) + q3_traj(i));
    ...
    q_matrix = [q1_traj_deg ; q2_traj_deg ; q3_traj_deg];
    q1_series = timeseries(q1_traj_deg , t); %first column
    q2_series = timeseries(q2_traj_deg , t); %second column
    q3_series = timeseries(q3_traj_deg , t); %third column
